...
public class ViewPager extends ViewGroup {
	...
	private int currentDuration = SCROLL_PORTRAIT_DURATION;
	...
	public void setCurrentItemInternal(int item, boolean smoothScroll,
		...
		populate();
		if (smoothScroll) {
			smoothScrollTo(getWidth() * item, 0);// 平滑第滚动到指定页面，scrollBy是直接(瞬间，立即)滚动指定距离
			if (mOnPageChangeListener != null) {
				mOnPageChangeListener.onPageSelected(mLastItem, item);
			}
		} else {
			if (mOnPageChangeListener != null) {
				mOnPageChangeListener.onPageSelected(mLastItem, item);
	...
	void smoothScrollTo(int x, int y) {
		...
		mScroller.startScroll(sx, sy, dx, dy, currentDuration);
		invalidate();
	}
	...
	public void notifyOrientationChanged(int orientation) {
		...
		case Configuration.ORIENTATION_PORTRAIT:
			currentDuration = SCROLL_PORTRAIT_DURATION;
			break;
		case Configuration.ORIENTATION_LANDSCAPE:
			currentDuration = SCROLL_LANDSCAPE_DURATION;
			break;
		default:
			currentDuration = SCROLL_PORTRAIT_DURATION;
		}
	...
	public void notifyPageSelected() {
		if (mOnPageChangeListener != null) {
			mOnPageChangeListener.onPageSelected(mLastItem, mCurItem);
		}
	}
		...
		if (mScrollingCacheEnabled != enabled) {
			...
			if (USE_CACHE) {
				...
				for (int i = 0; i < size; ++i) {
					final View child = getChildAt(i);
					if (child.getVisibility() != GONE) {
						child.setDrawingCacheBackgroundColor(Color.BLACK);
